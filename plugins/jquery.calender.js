		  ...
		  _this.$calender.find('.row').on('click','span.list span',function(event){
			...
			if($(this).attr('disabled')){
				...
				if(index == 0){
					...
					$(this).addClass('on')
					// 切换年份时，默认选中全年
					_this.$calender.find('.row:eq(1) span.list span:eq(0)').addClass('on');
				}else if(index == 3){
					// 点击月份后直接选中1月至点击的月份
...
//					}
	  ...
	  formatterResult:function(type,val){
		  ...
		  function getYljResult(){
			  ...
			  _this.$result.monthRange.push(1)
		  ...
		  function getDefaultResult(){
			  ...
			  if(_this.settings.tjqId){
				 ...
				 defaultVal = _this.$result.year+'全年'
				 _this.$input.attr('value',defaultVal).val(defaultVal)
	  ...
	  getResultByTjqid:function(tjqid){
	      ...
	      this.$input.data('value',$.extend({},this.$result));
	  },
	  /**
	   * 根据月份获取当前所处季度
	   */
